...
namespace Assignment_2
{
    ...
    public class Course
    {
        ...
        public Dictionary<uint, List<string>> grades = new Dictionary<uint, List<string>>();
    ...
    static class Program
    {
        ...
        //Adds grades of students to corresponding course
        public static void addGrade(string line)
        {
            string[] gradeArr = line.Split(',');
            uint id = Convert.ToUInt32(gradeArr[0]);
            string depCode = gradeArr[1];
            ushort courseNum = ushort.Parse(gradeArr[2]);
            string grade = gradeArr[3];

            foreach(Course c in Globals.coursePool){

                if(c.departmentCode.Equals(depCode) && c.courseNumber == courseNum)
                {
                    List<string> tempList = new List<string>();
                    tempList.Add(grade);
                    tempList.Add(depCode);
                    tempList.Add(gradeArr[2]);

                    if(!c.grades.ContainsKey(id))
                        c.grades.Add(id, tempList);
                }
        ...
        static void Main()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("ADDING STUDENTS...");
            ...
            {
                System.Diagnostics.Debug.WriteLine("ADDING COURSES...");
                while ((inLine = inFile.ReadLine()) != null)
                {
            ...
            //Loops through grades file and adds grades to corresponding dictionary
            using (StreamReader inFile = new StreamReader(@"..\..\grades.txt"))
            {
                System.Diagnostics.Debug.WriteLine("ADDING GRADES...");
